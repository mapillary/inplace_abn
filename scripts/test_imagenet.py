...
parser.add_argument(
    "--crop", "-c", metavar="N", type=int, default=224, help="crop size"
)
parser.add_argument(
    "--scale",
    "-s",
...
def main():
    ...
    val_transforms = utils.create_test_transforms(
        conf["input"], args.crop, args.scale, args.ten_crops
    )

    batch_size = (
        conf["optimizer"]["batch_size"]
        if not args.ten_crops
        else conf["optimizer"]["batch_size"] // 10
    )
